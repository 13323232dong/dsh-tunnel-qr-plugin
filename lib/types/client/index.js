...
export function apply(ctx) {
    ...
    const username = dialog.querySelector('[data-tunnel-username]');
    const password = dialog.querySelector('[data-tunnel-password]');
    const passwordToggle = dialog.querySelector('.dsh-tunnel-qr-password-toggle');
    if (username === null || password === null || passwordToggle === null)
        throw new Error('ui-tunnel-qr: missing credential fields');
    let passwordValue = '';
    ...
    const open = () => {
        dialog.hidden = false;
        void loadAccess();
        dialog.querySelector('.dsh-tunnel-qr-close')?.focus();
    };
    const onDialogClick = (event) => {
        const target = event.target;
        if (target instanceof HTMLElement && (target.matches('[data-qr-close]') || target.matches('.dsh-tunnel-qr-close')))
            close();
    };
