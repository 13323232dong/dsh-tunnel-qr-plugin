...
function targetHeaders(request, targetHost, targetPort) {
    ...
    for (const [name, value] of Object.entries(request.headers)) {
        if (HOP_BY_HOP.has(name) || connectionTokens.has(name) || name === 'host' || name === 'origin')
            continue;
        if (value !== undefined)
            headers[name] = value;
    }
...
async function readLoginToken(request) {
    ...
    try {
        ...
        if (typeof value !== 'object' || value === null || typeof value.token !== 'string') {
...
export class AuthenticationProxy {
    ...
    async start() {
        if (this.server !== undefined)
            throw new Error('authentication proxy already started');
    ...
    forwardHttp(request, response) {
        ...
        upstream.on('error', () => {
            if (response.headersSent)
                ...
                json(response, 503, { error: 'target-unavailable' });
        });
