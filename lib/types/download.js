...
async function materializeExecutable(archivePath, temporaryDirectory, artifact) {
    ...
    try {
        ...
        if (!file.isFile() || file.isSymbolicLink())
            throw new Error('expected a regular executable file');
    }
    catch (error) {
...
async function ensureOnce(options) {
    ...
    try {
        ...
        if (materialized !== staged)
            await writeFile(staged, await readFile(materialized), { flag: 'wx' });
        ...
        try {
            ...
            if (movedExisting)
                await rename(backup, paths.executable);
            throw error;
        }
        const metadata = {
...
export function ensureCloudflared(options) {
    const paths = cachePaths(options);
