...
export function apply(ctx, config) {
    ctx.effect(() => ctx.webServer.register({
        ...
        handler: async (request, response) => {
            ...
            response.end(request.method === 'HEAD' ? undefined : QR_IMAGE);
        },
    }), 'ui-tunnel-qr: image route');
    const accessJson = JSON.stringify({ username: config.username, password: config.password });
    ctx.effect(() => ctx.webServer.register({
        kind: 'exact',
        ...
        handler: (request, response) => {
            ...
            response.end(request.method === 'HEAD' ? undefined : accessJson);
        },
