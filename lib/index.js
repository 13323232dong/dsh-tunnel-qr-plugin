...
var QrCredentials = class {
	...
	validateSession(session, generation) {
		...
		const valid = digestMatches && record.generation === generation && record.expiresAt > this.now();
	...
	invalidateGeneration(generation) {
		...
		for (const [key, record] of this.sessions) if (record.generation === generation) this.sessions.delete(key);
	}
